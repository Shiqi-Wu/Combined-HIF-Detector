...
sys.path.append("./src")
import common.dataloader as dataloader_module
from lstm.model import LSTMClassifier
from dynamic.eval import KnownControlClassifier, calculate_top_k_accuracy, compute_loss_from_logits
from dynamic.eval import DynamicSystemDataset
from koopman.inverse_eval import (
    KoopmanPhiModel,
...
def resolve_config_path(config_dir: Path, requested: Optional[str], default_rel: str) -> str:
    """Resolve a config path, allowing either an explicit path or a path relative to config_dir."""
    candidate = requested or default_rel
    candidate_path = Path(candidate)
    if candidate_path.exists():
        return str(candidate_path)

    rel_candidate = config_dir / candidate
    if rel_candidate.exists():
        return str(rel_candidate)

    default_candidate = config_dir / default_rel
    if requested is None and default_candidate.exists():
        return str(default_candidate)

    raise FileNotFoundError(
        f"Could not resolve config path '{candidate}'. "
        f"Tried '{candidate_path}' and '{rel_candidate}'."
    )


def main():
    parser = argparse.ArgumentParser(
        description="Evaluate LSTM, dynamic-system, and Koopman baselines with shared reporting.",
    ...
    parser.add_argument(
        ...
        type=str,
        default='configs',
        help='Base directory used to resolve relative config paths'
    )
    parser.add_argument(
        '--config',
        type=str,
        default='lstm/classifier.json',
        help='LSTM config path, absolute or relative to --config_dir'
    )
    ...
    parser.add_argument('--model_path', type=str, default='best_model.pth',
                        help='Path to trained LSTM checkpoint (used by method=lstm and method=all)')
    parser.add_argument('--save_csv', type=str, default='evaluation_results.csv',
                        help='Base path for summary CSV output; window metrics are saved beside it')
    parser.add_argument('--window_lengths', type=int, nargs='+', default=None,
                        help='Optional list of explicit window lengths to evaluate')
    parser.add_argument('--topk', type=int, nargs='+', default=[1, 2, 3],
                        help='Top-k values to compute in summary and windowed metrics')
    parser.add_argument('--koopman_config', type=str, default='koopman/phi.json',
                        help='Koopman config path, absolute or relative to --config_dir')
    parser.add_argument('--koopman_checkpoint', type=str, default=None,
                        help='Checkpoint path for Koopman Phi model (defaults to train.save_path in Koopman config)')
    args = parser.parse_args()

    config_dir = Path(args.config_dir)
    lstm_config_path = resolve_config_path(config_dir, args.config, 'lstm/classifier.json')
    koopman_config_path = resolve_config_path(config_dir, args.koopman_config, 'koopman/phi.json')
    ...
    for method in methods:
        ...
        if method == 'lstm':
            ...
            evaluate_koopman(koopman_config_path, args.koopman_checkpoint,
